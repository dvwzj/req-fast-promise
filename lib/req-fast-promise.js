...
class ReqFastPromise {
  ...
  extends(options) {
    return this.create(_lodash.default.merge({}, this.defaults, options));
  }
  ...
  request(method, url) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (options.baseURL && !url.startsWith('http')) {
        url = "".concat(_lodash.default.trim(options.baseURL, '/'), "/").concat(_lodash.default.trim(url, '/'));
      }

      if (options.params && _lodash.default.size(_lodash.default.keys(options.params))) {
        var uri = _url.default.parse(url);
